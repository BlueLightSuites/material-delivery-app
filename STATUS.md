...
- **No *live* tracking.** A contractor can see status, but only by refreshing — nothing is pushed. Driver position is readable on the device and never leaves it: `src/hooks/useRealtimeLocation.ts` is still empty, Supabase Realtime is an unused dependency, and no column stores a driver's position. There's also no map anywhere yet; `src/components/map/MapView.tsx` remains an empty file despite `react-native-maps` being installed.
