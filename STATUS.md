...
- **Edit / cancel a request** (`src/screens/Contractor/NewRequest.tsx`, `Tracking.tsx`) — a contractor can edit a request while it's still pending and cancel it while pending or assigned, but not once it's in transit (the driver is physically carrying the load by then). Editing reuses the creation wizard via a `requestId` param, opening on a summary with per-section Edit controls and a diff showing each changed field's previous value. Cancellation goes through the `cancel_delivery_request` RPC; the driver is told via push and, more reliably, a notice that stays in their jobs list until acknowledged (`driver_ack_cancelled_at`).
  - The same change **narrowed the update policy**, which previously allowed a contractor to rewrite any column on their own request at any status — including reverting an in-transit job to pending or unassigning its driver. Editing is now confined to pending requests and must leave them pending.
- **Two-sided ratings** (`src/components/job/Rating.tsx`, `src/services/api/ratings.ts`) — both parties rate each other once per completed delivery, one to five stars with an optional comment, skippable. The contractor sees their driver's average from the moment one accepts; an unrated person reads as "New driver" rather than 0.0 stars. `submit_rating` derives the ratee from the delivery rather than trusting a parameter, and a unique constraint per (delivery, rater) makes "already rated?" race-free. Averages are denormalised onto `users` by a trigger. The cross-party read stayed narrow — `get_counterparty_rating` exposes an average and count for one delivery, not general access to driver profiles.
- **Contractor tracking** (`src/screens/Contractor/Tracking.tsx`) — fetches the request by id and shows a status badge, a plain-language line for the current step, and a four-step progress timeline (`pending` → `assigned` → `in_transit` → `completed`), plus route, trailer requirement, request time, and notes. Subscribes over Supabase Realtime for live updates, falling back to polling if the channel fails, and also refetches on focus and pull-to-refresh. Status colors and labels come from `src/models/deliveryStatus.ts`, shared with `RequestList` so the two can't disagree.
...
- **The toolchain is two years ahead of the SDK** ([#42](https://github.com/BlueLightSuites/material-delivery-app/issues/42)). Expo SDK 48 (early 2023) against Xcode 16.4 and an iOS 26 test device. Consequences: local builds to a physical device are impossible (Xcode 16.4 ships the iOS 18.5 SDK), so device builds go through EAS; and SDK 48-era native modules can fail to compile — `expo-device` already did. Simulator builds work fine. When a build fails with a bare `error code 65`, run `xcodebuild` directly to see the real message, which Expo's wrapper swallows.
- **Push notifications are code-complete but unverified** ([#9](https://github.com/BlueLightSuites/material-delivery-app/issues/9)). The Apple side is fully configured — App ID, entitlement, APNs key `MSS2J73K3V` uploaded to EAS — but tokens are never issued to a simulator, so nothing has run for real. Needs one EAS device build.
